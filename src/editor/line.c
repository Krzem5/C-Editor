...
void update_line(editor_t* e,uint32_t i,uint8_t fl){
	...
	if (fl&EDITOR_LINE_FLAG_RENDER){
		...
		l->_bfl=e->f._ll_sz+l->l+(i==e->f.cy?STR_LEN(EDITOR_UI_START_STR_HIGHLIGHT):STR_LEN(EDITOR_UI_START_STR))+STR_LEN(" "EDITOR_UI_DEFAULT_FG_COLOR);
		l->_ws_c=0;
	}
	uint32_t j=0;
	uint32_t n=0;
	fl|=(e->f.cy==i&&!(fl&EDITOR_LINE_FLAG_UPDATE_CURSOR_POS)?EDITOR_LINE_FLAG_HAS_CURSOR:0)|(fl&EDITOR_LINE_FLAG_RENDER?EDITOR_LINE_FLAG_IN_WHITESPACE:0);
	for (;j<l->l;j++){
		uint8_t c=*(l->dt+j);
		if ((fl&EDITOR_LINE_FLAG_UPDATE_CURSOR_POS)&&n>=e->f._cx){
			fl&=~EDITOR_LINE_FLAG_UPDATE_CURSOR_POS;
			e->f.cx=j;
			e->f._cx=n;
		}
		else if ((fl&EDITOR_LINE_FLAG_HAS_CURSOR)&&e->f.cx==j){
	...
	uint32_t k=_copy_str(l->_bf,(i==e->f.cy?EDITOR_UI_START_STR_HIGHLIGHT:EDITOR_UI_START_STR));
	char bf[10];
	j=0;
	uint32_t v=i+1;
	while (v){
